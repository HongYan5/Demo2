...
function deleteProvider(obj){
	$.ajax({
		type:"GET",
		url:path+"/jsp/providerDelete.do",
...
$(function(){
	$(".viewProvider").on("click",function(){
		...
		var obj = $(this);
		window.location.href=path+"/jsp/getprovider.do?mode=0&proid="+ obj.attr("proid");
	});
	
	$(".modifyProvider").on("click",function(){
		var obj = $(this);
		window.location.href=path+"/jsp/getprovider.do?mode=1&proid="+ obj.attr("proid");
	});
	...
	$(".deleteProvider").on("click",function(){
		providerObj = $(this);
		changeDLGContent("你确定要删除供应商1【"+providerObj.attr("proname")+"】吗？");
		openYesOrNoDLG();
	});
